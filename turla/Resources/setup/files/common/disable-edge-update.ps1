# ---------------------------------------------------------------------------
# disable-edge-update.ps1 - disable edge update
...
# Usage: disable-edge-update.ps1

# ---------------------------------------------------------------------------


Write-Host "Creating registry key to disable Edge Update"
...
if ($null -eq (Get-ItemProperty @RegItem -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "set EdgeUpdate value to 0"
}
