# ---------------------------------------------------------------------------
# disable-edge-first-run.ps1 - disable edge first run
...
# Usage: disable-edge-first-run.ps1

# ---------------------------------------------------------------------------



Write-Host "Creating registry key to disable Edge first run"
...
if ($null -eq (Get-ItemProperty @RegItem -ErrorAction SilentlyContinue)) {
    New-ItemProperty @RegItem -Value "1" -PropertyType DWord -Force | Out-Null
    Write-Host 'added Registry value' -f red
} else {
    set-ItemProperty @RegItem -Value "1"
    Write-Host "set hideFirstRunExperience value to 1"
}
