...
var handler = &exaramel.ExaramelHandler{}
...
func startExaramelHandler(t *testing.T) {
	if err := handler.StartHandler(restAPIlistenHost, configEntry); err != nil {
		t.Errorf("Error when starting exaramel handler: %s", err.Error())
	}
	time.Sleep(50 * time.Millisecond)
}

func stopExaramelHandler(t *testing.T) {
	if err := handler.StopHandler(); err != nil {
		t.Errorf("Error when stopping exaramel handler: %s", err.Error())
	}
	time.Sleep(50 * time.Millisecond)
}
...
func TestStartStopTrickBotHandler(t *testing.T) {
	startExaramelHandler(t)
	stopExaramelHandler(t)
}
...
func TestPostFileToServer(t *testing.T) {
	...
	_, err = SetTask("get ./test_payloads/hello_world.elf test_file.elf", exampleAuth.guid)
	if err != nil {
		t.Error(err)
	}
