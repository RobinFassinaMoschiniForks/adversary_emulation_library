...
// Represents the HTTP C2 handler. Will implement the util.Handler interface.
type HttpHandler struct {}

// Creates and adds the HttpHandler to the map of available C2 handlers.
func init() {
	util.AvailableHandlers["simplehttp"] = &HttpHandler{}
}

// StartHandler starts the simplehttp server
func (h *HttpHandler) StartHandler(restAddress string, configEntry config.HandlerConfigEntry) error {
	listenAddress, err := config.GetHostPortString(configEntry)
	if err != nil {
		return err
	}
	logger.Info("Starting simple HTTP Handler")
...
// StopHandler stops the simplehttp server
func (h *HttpHandler) StopHandler() error {
...
func forwardPostTaskOutput(guid string, output string) (string, error) {
	url := "http://" + RestAPIaddress + "/api/v1.0/session/" + guid + "/task/output"
